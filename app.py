...
# 修复：统一使用时间戳，而不是datetime对象
if 'last_refresh_time' not in st.session_state:
    st.session_state.last_refresh_time = time.time()  # 使用时间戳
...
with col2:
    # 修复：使用time.time()而不是datetime.now()
    if st.button("🔄 刷新", use_container_width=True, key='manual_refresh_top'):
